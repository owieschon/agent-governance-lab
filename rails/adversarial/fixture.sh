...
cp "$HOST"/rails/verifier/verify.sh "$HOST"/rails/verifier/remote_ref.sh \
   ...
   "$HOST"/rails/verifier/snapshot.sh "$HOST"/rails/verifier/snapshot.py "$HOST"/rails/verifier/basis.sh "$HOST"/rails/verifier/oracle_independence.py \
   "$HOST"/rails/verifier/exercised_assertions.py \
   "$HOST"/rails/verifier/adjudicate.py "$HOST"/rails/verifier/why.sh \
   "$HOST"/rails/verifier/why.py \
   "$HOST"/rails/verifier/receipt.sh "$HOST"/rails/verifier/receipt.py "$HOST"/rails/verifier/nextstep_lint.py \
   "$HOST"/rails/verifier/status.sh "$HOST"/rails/verifier/status.py "$HOST"/rails/verifier/doctor.sh \
   "$HOST"/rails/verifier/demo.sh "$HOST"/rails/verifier/init.sh \
   "$HOST"/rails/verifier/eject.sh "$HOST"/rails/verifier/freshness.py \
   "$HOST"/rails/verifier/scoreboard.sh "$HOST"/rails/verifier/scoreboard.py "$HOST"/rails/verifier/scoreboard_metrics.sql \
   "$HOST"/rails/verifier/flaky_triage.sh "$HOST"/rails/verifier/flaky_triage.py \
   "$HOST"/rails/verifier/render_review_summary.py \
   "$HOST"/rails/verifier/pre-push.sh \
   "$SB/rails/verifier/"
# posture files (role kernel)
if [ -d "$HOST/rails/verifier/postures" ]; then
  mkdir -p "$SB/rails/verifier/postures"
  cp "$HOST"/rails/verifier/postures/*.json "$SB/rails/verifier/postures/"
fi
# reviewer agent definition
if [ -f "$HOST/.claude/agents/reviewer.md" ]; then
  cp "$HOST/.claude/agents/reviewer.md" "$SB/.claude/agents/reviewer.md"
fi
# observers + notify (Job 4): runner, transforms, definitions, dry-run fixtures
if [ -d "$HOST/rails/observers" ]; then
  mkdir -p "$SB/rails/observers/state" "$SB/rails/observers/fixtures"
  cp "$HOST"/rails/observers/run_observer.sh "$HOST"/rails/observers/run_observer.py \
     "$HOST"/rails/observers/extract.py \
     "$HOST"/rails/observers/*.json "$SB/rails/observers/" 2>/dev/null || true
  cp "$HOST"/rails/observers/fixtures/*.jsonl "$SB/rails/observers/fixtures/" 2>/dev/null || true
  chmod +x "$SB/rails/observers/run_observer.sh" 2>/dev/null || true
fi
if [ -f "$HOST/rails/notify/notify.sh" ]; then
  mkdir -p "$SB/rails/notify"
...
cp "$HOST/rails/verifier/pre-push.sh" "$SB/.git/hooks/pre-push"
chmod +x "$SB/.git/hooks/pre-push"
...
echo "[]" > rails/verifier/flaky_lane.json
...
# oracle snapshot: test/fixture hashes as of approval (scorer-integrity baseline)
