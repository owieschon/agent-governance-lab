...
expect_blocked "file-edit the oracle_independence scanner" "$(agent_edit "$SANDBOX/rails/verifier/oracle_independence.py")"
expect_blocked "file-edit the exercised_assertions scanner" "$(agent_edit "$SANDBOX/rails/verifier/exercised_assertions.py")"
...
expect_blocked "file-edit the receipt verifier"   "$(agent_edit "$SANDBOX/rails/agl/receipt.py")"
expect_blocked "file-edit the canonical CLI"      "$(agent_edit "$SANDBOX/bin/agl")"
...
expect_blocked "shell-overwrite the receipt verifier" "$(agent_bash 'echo pass > rails/agl/receipt.py')"
expect_blocked "shell-overwrite the canonical CLI" "$(agent_bash 'echo pass > bin/agl')"
...
expect_blocked "delete the oracle snapshot (disables oracle_integrity)" "$(agent_bash 'rm rails/dispatches/active/D-test/.oracle_snapshot.json')"
expect_blocked "edit the oracle snapshot"         "$(agent_edit "$SANDBOX/rails/dispatches/active/D-test/.oracle_snapshot.json")"
