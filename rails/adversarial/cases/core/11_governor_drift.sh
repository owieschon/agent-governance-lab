...
# Untracked-inside-the-fingerprint (added 2026-06-10, INC process_gap): the
# fingerprint hashes the FILESYSTEM, so an untracked file joins the stamp
# without ever appearing in a reviewed diff. The drift gate still fires on it
# (no silent un-proven governor), and --untracked makes the silent inclusion
# NAMEABLE at stamp/doctor time. Both directions: named when present, silent
# when the scope is clean.
cd "$SANDBOX"
_assert "clean scope -> no untracked files reported" "" \
  "$(python3 rails/verifier/fingerprint.py . --untracked)"
echo "# planted, never committed" > rails/verifier/planted_helper.py
_assert "a planted untracked file changes the fingerprint (drift gate fires)" \
  1 "$(run_verify)"
_assert "--untracked names the planted file" 1 \
  "$(python3 rails/verifier/fingerprint.py . --untracked | grep -c planted_helper.py)"
rm -f rails/verifier/planted_helper.py
_assert "removed -> verify PASS again (no false-positive)" 0 "$(run_verify)"
finish
